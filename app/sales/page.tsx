...
export default function SalesPage() {
  ...
  const [clientSuggestions, setClientSugs]    = useState<string[]>([])
  ...
  const fetchClientNames = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq('user_id', profile.id)
      .order('name')
    setClientSugs((data || []).map((c: { name: string }) => c.name))
  }, [profile])

  useEffect(() => {
    fetchSales()
    fetchClientNames()
  }, [fetchSales, fetchClientNames])
  ...
  const handleSubmit = async () => {
    ...
    // Upsert client — use maybeSingle to avoid error when not found
    ...
    if (existClient) {
      await supabase
        .from('clients')
        .update({
          total_spent: (existClient.total_spent || 0) + salePrice,
          updated_at: new Date().toISOString(),
        })
        .eq('id', existClient.id)
    } else {
      await supabase.from('clients').insert({
        user_id: profile.id,
        name: form.client_name.trim(),
        total_spent: salePrice,
      })
    }
    ...
    if (error) {
      toast.err("Erreur lors de l'enregistrement")
      return
    }

    const benefit = salePrice - buyPrice - deliveryFee
    toast.ok(`Vente enregistrée ! Bénéfice : ${fmtCFA(benefit)} 💰`)
    setShowModal(false)
    setForm(emptyForm())
    fetchSales()
    fetchClientNames()
  ...
  const PERIOD_LABELS: Record<Period, string> = {
    day: 'Jour', month: 'Mois', year: 'Année', all: 'Tout',
  }
    ...
    >
      ...
      <div className="g3" style={{ marginBottom: 18 }}>
        ...
        <div className="stat-card">
          ...
          <div
            ...
            style={{ color: totalProfit >= 0 ? 'var(--green)' : 'var(--red)' }}
          >
            {fmtCFA(totalProfit)}
          </div>
      ...
      <div className="card">
        <div className="card-title">
          💰 Historique — {fmt(filtered.length)} vente{filtered.length !== 1 ? 's' : ''}
        </div>
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="es">
              {search ? 'Aucun résultat pour cette recherche' : 'Cliquez sur "+ Nouvelle vente" pour commencer'}
            </div>
          </div>
        ) : (
                  ...
                  <tr key={s.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div
                          className="avatar"
                          style={{ background: avatarColor(s.client_name), width: 28, height: 28, fontSize: 11 }}
                        >
                    ...
                    <td style={{ color: 'var(--text3)', fontFamily: 'var(--mono)', fontSize: 12 }}>
                      {s.delivery_fee ? fmtCFA(s.delivery_fee) : '—'}
                    </td>
                    <td
                      className={`money ${(s.profit || 0) >= 0 ? 'money-accent' : 'money-red'}`}
            ...
            <div className="modal-body">
              ...
              <div className="g2">
                <div className="field">
                  ...
                  <select
                    value={form.sector}
                    onChange={e => { setF('sector', e.target.value); setF('category', '') }}
                  >
              ...
              <div className="field">
                ...
                <input
                  placeholder="Ex: Laptop HP 15s, Réparation écran..."
                  value={form.product}
              ...
              <div className="g3">
                ...
                <div className="field">
                  ...
                  <input
                    ...
                    onChange={e => setF('buy_price', e.target.value)}
                  />
                </div>
                <div className="field">
                  ...
                  <input
                    ...
                    onChange={e => setF('delivery_fee', e.target.value)}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
                ...
                <button
                  ...
                  disabled={saving}
                >
                  {saving ? <><span className="spinner" /> Enregistrement...</> : '✓ Enregistrer la vente'}
                </button>
              </div>
            </div>
          </div>
